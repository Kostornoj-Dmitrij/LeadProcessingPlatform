...
- **Образовательная:** Демонстрация современных архитектурных паттернов (EDA, хореографируемая Saga, Outbox, Inbox) на практическом примере.
...
Координация распределённой транзакции обработки лида (обогащение → скоринг → распределение) реализована через **хореографию**, а не централизованную оркестрацию.

**Как работает:**
- Lead Service управляет конечным автоматом агрегата `Lead`
- Каждый микросервис публикует события о результате своей работы
- Другие сервисы реагируют на эти события самостоятельно
- При ошибке (например, `DistributionFailed`) сервисы-участники выполняют компенсирующие транзакции
...
**Важно:** Перед первым нагрузочным тестом рекомендуется выполнить «прогрев» системы - запустить короткий тест (например, 20 RPS на 10 секунд). Это необходимо для первоначального заполнения пулов соединений PostgreSQL, кэша Schema Registry и прогрева JIT-компиляции.
