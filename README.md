...
**Важно:** Перед первым нагрузочным тестом рекомендуется выполнить «прогрев» системы - запустить короткий тест (например, 30 RPS на 20 секунд). Это необходимо для первоначального заполнения пулов соединений PostgreSQL, кэша Schema Registry и прогрева JIT-компиляции.
