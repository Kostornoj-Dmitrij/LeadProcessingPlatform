...
/// <summary>
/// Сценарий с ошибкой распределения лидов
/// </summary>
public static class DistributionFailureScenario
{
    ...
    public static ScenarioProps Create(string apiGatewayUrl, int targetRps, int durationSeconds, LeadGenerator generator)
    {
        ...
        return Scenario.Create("distribution_failure", async context =>
            {
                var token = await AuthHelper.GetTokenAsync(apiGatewayUrl);

                var lead = TestData.CreateDistributionFailureLead();
                ...
                try
                {
                    ...
                    if (json.RootElement.TryGetProperty("id", out var idElement))
                    {
                        ...
                        _generator?.TrackCreatedLead(leadId, ExpectedScenarioPath.DistributionFailure);
                    }
                }
                catch (Exception ex)
                {
