...
/// <summary>
/// Сценарий с ошибкой скоринга лидов
/// </summary>
public static class ScoringFailureScenario
{
    ...
    public static ScenarioProps Create(string apiGatewayUrl, int targetRps, int durationSeconds, LeadGenerator generator)
    {
        ...
        return Scenario.Create("scoring_failure", async context =>
            {
                var lead = TestData.CreateScoringFailureLead();
                ...
                try
                {
                    ...
                    if (json.RootElement.TryGetProperty("id", out var idElement))
                    {
                        ...
                        _generator?.TrackCreatedLead(leadId, ExpectedScenarioPath.ScoringFailure);
                    }
                }
                catch (Exception ex)
                {
