...
{
    private const string NotificationType = "LeadQualified";
    ...
    public void Setup()
    {
        ...
        _sut = new LeadQualifiedEventHandler(
    ...
    [Test, AutoData]
    public async Task Handle_WhenEnrichedDataIsNull_ShouldStillSendNotification(
        [WithValidLeadQualifiedEvent] LeadQualified @event,
        List<Notification> notifications,
        string subject,
        string body)
    {
        @event.EnrichedData = null;
        var notificationSetMock = CreateMockDbSet(notifications);
    ...
    [Test, AutoData]
    public async Task Handle_ShouldPassCorrectVariablesWithEnrichedData(
        [WithValidLeadQualifiedEvent] LeadQualified @event,
        List<Notification> notifications,
        string subject,
        string body)
    {
        ...
        Assert.That(capturedVariables!["Score"], Is.EqualTo(@event.Score.ToString()));
        Assert.That(capturedVariables!["Industry"], Is.EqualTo(@event.EnrichedData?.Industry ?? "Unknown"));
        Assert.That(capturedVariables!["CompanySize"], Is.EqualTo(@event.EnrichedData?.CompanySize ?? "Unknown"));
    }
}