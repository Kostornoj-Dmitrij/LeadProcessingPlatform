...
using ScoringService.Host.Extensions;
using ScoringService.Host.Middleware;
using ScoringService.Host.Options;
...
builder.Services.AddOpenTelemetryConfiguration(builder.Configuration, "ScoringService");
...
builder.Services.AddOpenApiDocument(config =>
{
    ...
    config.Title = "Scoring Service API";
    config.Version = "v1";
    config.Description = "API for scoring B2B leads";
});
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        config.DocumentTitle = "Scoring Service API";
        config.Path = "/swagger";
    });
}
else
