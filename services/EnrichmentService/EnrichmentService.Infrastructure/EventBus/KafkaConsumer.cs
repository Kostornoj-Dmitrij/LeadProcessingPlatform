...
public class KafkaConsumer : BackgroundService, IKafkaConsumer
{
    ...
    private static readonly ActivitySource ActivitySource = new("EnrichmentService.KafkaConsumer");

    public KafkaConsumer(IConfiguration configuration, IServiceScopeFactory scopeFactory, ILogger<KafkaConsumer> logger)
    {
        ...
        var consumerConfig = new ConsumerConfig
        {
            ...
            GroupId = configuration["Kafka:GroupId"] ?? "enrichment-service",
    ...
    private async Task ProcessMessageWithRetryAsync(ConsumeResult<string, string> consumeResult, CancellationToken cancellationToken)
    {
        ...
        while (attempt < _maxRetryAttempts)
        {
            ...
            {
                attempt++;
                _logger.LogWarning(ex, "Transient error processing message (attempt {Attempt}/{MaxAttempts})", attempt, _maxRetryAttempts);

                if (attempt >= _maxRetryAttempts)
                {
                    _logger.LogError(ex, "Max retry attempts reached. Moving to DLQ. Topic: {Topic}, Offset: {Offset}", consumeResult.Topic, consumeResult.Offset);
            ...
            {
                _logger.LogError(ex, "Non-transient error. Moving to DLQ. Topic: {Topic}, Offset: {Offset}", consumeResult.Topic, consumeResult.Offset);
                await MoveToDeadLetterQueueAsync(consumeResult, ex);
                return;
            }
        }
    ...
    private async Task MoveToDeadLetterQueueAsync(ConsumeResult<string, string> consumeResult, Exception exception)
    {
        ...
        _logger.LogWarning("Message moved to DLQ. Original topic: {Topic}, Offset: {Offset}, Error: {Error}",
            consumeResult.Topic, consumeResult.Offset, exception.Message);
    }
